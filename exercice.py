...
def convert_to_bytes(samples):
    ...
    # 1. Limiter (ou clamp/clip) les échantillons
    clipped = np.clip(samples, -1, 1)
    # 2. convertier en entier 16-bit signés
    init_samples = (clipped * MAX_INT_SAMPLE_VALUE).astype("<i2")
    # 3. convertir en bytes
    sample_bytes = init_samples.tobytes()
    return sample_bytes
...
def main():
    ...
    if True:
        with wave.open("output/perfect_fifth.wav", "wb") as writer:
            ...
            samples1 = sine(220, 1, 30.0)
            samples2 = sine(220 * (3 / 2), 0.9, 30.0)
